// https://github.com/bumbu/freeboard-plugins/tree/master/googleSheets
// https://buglabs.tumblr.com/post/128201590116/simple-freeboard-data-storage-with-google-sheets
    ...
    function processEntries(entries) {
        if (_.isArray(entries) && entries.length > 0) {
            ...
            for (var i = 0; i < entries.length; i++) {
                ...
                for (header of headers) {
                    if (header == 'title') {
                        dataset['data'][i]['title'] = entries[i].title.$t;
                    } else {
                        dataset['data'][i][header] = entry[header] || null;
                    }
                }
            }
        } else {
...
freeboard.loadDatasourcePlugin({
    ...
    "display_name": "Google Sheet Data - CUSTOM",
    "settings": [
        {
            "name": "sheet_key",
